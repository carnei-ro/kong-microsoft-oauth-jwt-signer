...
config:
	...
	@curl -i -X POST http://localhost:8001/services/oauth/plugins -F "name=${NAME}" -F "config.callback_uri=/_oauth" -F "config.client_secret=$$(grep client_secret cred | awk '{print $$NF}')" -F "config.client_id=$$(grep client_id cred | awk '{print $$NF}')" -F "config.secure_cookies=false" -F "config.http_only_cookies=false" -F "config.private_key_id=4a50b478-b164-11e9-a2a3-2a2ae2dbcce4"
